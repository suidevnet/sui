...
impl KeyPair {
    pub fn public(&self) -> PublicKey {
        match self {
            KeyPair::Ed25519KeyPair(kp) => PublicKey::Ed25519KeyPair(
                Ed25519PublicKey::from_bytes(kp.public().as_bytes()).unwrap(),
            ),
            KeyPair::Secp256k1KeyPair(kp) => PublicKey::Secp256k1KeyPair(
                Secp256k1PublicKey::from_bytes(kp.public().as_bytes()).unwrap(),
            ),
        }
    }
}
...
impl signature::Signer<Signature> for KeyPair {
    fn try_sign(&self, msg: &[u8]) -> Result<Signature, signature::Error> {
        match self {
            KeyPair::Ed25519KeyPair(kp) => kp.try_sign(msg),
            KeyPair::Secp256k1KeyPair(kp) => kp.try_sign(msg),
        }
    }
}
...
impl EncodeDecodeBase64 for KeyPair {
    fn encode_base64(&self) -> String {
        ...
        match self {
            KeyPair::Ed25519KeyPair(kp) => {
                let kp1 = kp.copy();
                bytes.extend_from_slice(&[Ed25519SuiSignature::SCHEME.flag()]);
                bytes.extend_from_slice(kp.public().as_ref());
                bytes.extend_from_slice(kp1.private().as_ref());
            }
            KeyPair::Secp256k1KeyPair(kp) => {
                let kp1 = kp.copy();
                bytes.extend_from_slice(&[Secp256k1SuiSignature::SCHEME.flag()]);
                bytes.extend_from_slice(kp.public().as_ref());
                bytes.extend_from_slice(kp1.private().as_ref());
            }
        }
        ...
        match bytes.first() {
            Some(x) => {
                if x == &Ed25519SuiSignature::SCHEME.flag() {
                    ...
                    Ok(KeyPair::Ed25519KeyPair(<Ed25519KeyPair as From<
                        Ed25519PrivateKey,
                    >>::from(sk)))
                } else if x == &Secp256k1SuiSignature::SCHEME.flag() {
                    let sk =
                        Secp256k1PrivateKey::from_bytes(&bytes[1 + Ed25519PublicKey::LENGTH..])?;
                    Ok(KeyPair::Secp256k1KeyPair(<Secp256k1KeyPair as From<
                        Secp256k1PrivateKey,
                    >>::from(sk)))
                } else {
...
impl PublicKey {
    pub fn inner_into(&self) -> SuiAddress {
        match self {
            PublicKey::Ed25519KeyPair(kp) => kp.into(),
            PublicKey::Secp256k1KeyPair(kp) => kp.into(),
        }
    }
}
//
